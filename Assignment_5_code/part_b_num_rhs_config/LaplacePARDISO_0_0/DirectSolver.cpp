    ...
    const bool writeOutput)
{
    ...
    MKL_INT nrhs = NUM_RHS;   // Number of right hand sides
    ...
    using array_t = float (&) [NUM_RHS][XDIM][YDIM][ZDIM];
    float *xM = new float [NUM_RHS*XDIM*YDIM*ZDIM];
    float *fM = new float [NUM_RHS*XDIM*YDIM*ZDIM];
    
    array_t MultipleX = reinterpret_cast<array_t>(*xM);
    array_t MultipleF = reinterpret_cast<array_t>(*fM);

    for(int n_rhs=0; n_rhs<NUM_RHS;n_rhs++){
        for(int i=0; i<XDIM;i++){
            for(int j=0; j<YDIM;j++){
                for(int k=0;k<ZDIM;k++){
                    MultipleX[n_rhs][i][j][k] = x[i][j][k];
                    MultipleF[n_rhs][i][j][k] = f[i][j][k];
                }
            }
        }
    }
    ...
    PARDISO (pt, &maxfct, &mnum, &mtype, &phase, &n,
        matrix.GetValues(), matrix.GetRowOffsets(), matrix.GetColumnIndices(),
        &idum, &nrhs, iparm, &msglvl, static_cast<void*>(&MultipleF[0][0][0][0]), &MultipleX[0][0][0][0], &error);
    if ( error != 0 )
